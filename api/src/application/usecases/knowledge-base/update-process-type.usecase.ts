...
/**
 * Update Process Type Use Case
 * Updates an existing process type in the knowledge base
 */
@Injectable()
export class UpdateProcessTypeUseCase {
   ...
   * @param id Process type ID to update
   * @param input Update data
   * @returns Updated process type template
   */
  async execute(id: string, input: UpdateProcessTypeDto): Promise<ProcessTypeTemplateDto> {
    // Validate input
    this.validateInput(id, input);

    // Update process type through service
    const updated = await this.knowledgeBaseManagerService.updateProcessType(id, {
      name: input.name,
   ...
   */
  private validateInput(id: string, input: UpdateProcessTypeDto): void {
    // Validate ID
    ...
    if (input.name !== undefined) {
      if (input.name.trim().length === 0) {
        throw new DomainException('Process type name cannot be empty');
      }
    ...
    // Validate common deliverables if provided
    if (input.commonDeliverables !== undefined) {
      if (!Array.isArray(input.commonDeliverables)) {
