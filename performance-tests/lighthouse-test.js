...
async function analyzePerformance(result) {
  ...
  return {
    ...
    opportunities: Object.values(lhr.audits)
      .filter(
        (audit) => audit.details?.type === "opportunity" && audit.score < 0.9
      )
      .map((audit) => ({
        title: audit.title,
...
async function generateReport(results) {
    ...
    (category) => {
      if (desktopResults.length > 0) {
        summary.averageScores.desktop[category] =
          desktopResults.reduce((sum, r) => sum + r[category], 0) /
          desktopResults.length;
      }
      if (mobileResults.length > 0) {
        summary.averageScores.mobile[category] =
          mobileResults.reduce((sum, r) => sum + r[category], 0) /
  ...
  for (const result of results) {
    ...
    await fs.writeFile(path.join(reportDir, filename), result.report[0]);
  }
...
async function runTests() {
  ...
  for (const { name, url } of urls) {
    ...
    try {
      ...
      results.push({
        ...
        bestPractices:
          desktopResult.lhr.categories["best-practices"].score * 100,
    ...
    try {
      ...
      results.push({
        ...
        bestPractices:
          mobileResult.lhr.categories["best-practices"].score * 100,
  ...
  if (failedTests.length > 0) {
    ...
    failedTests.forEach((test) => {
      console.log(
        `  - ${test.name} (${test.device}): ${test.performance.toFixed(1)}`
      );
    });
    process.exit(1);
  } else {
